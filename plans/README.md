...
| 010  | Purge dead code (`ProcessingSection`, `ProcessingQueuePanel`) + unify the 4 scattered processing-status types into `src/types/processing.ts` | P2 | S–M | — (extends 006) | DONE (2 orphans deleted; types+ProcessingStatus union+predicates centralized; queue/hook rewired; `as string` casts gone) |
| 011  | Unify the OpenRouter client into one `openRouterChat()` in `lib/llm.ts` (collapse 4 fetch copies + 3 type copies) | P2 | M | 003 (soft) | DONE (single `openRouterChat` transport in `lib/llm.ts`; 4 inline fetches + 3 type copies removed; each caller keeps its own mapping — 502/throw/content preserved; +10 tests) |
| 012  | Consolidate URL + timezone utils into single authorities (fix `normalizeUrl` interior-whitespace bug, URLPill ×3 dedup, normalize detected URLs, tz abbrev tables ×3→1, merge `timezoneResolver`) | P2 | M | 003 (soft) | DONE (normalizeUrl space→`%20` fixed; `safeParseUrl`/`getUrlDisplayParts` added; URLPill parses once, dead `isMeetupURL`/`isValidUrl` gone; detect-urls route normalizes; `resolveTimezone`+known-set absorbed into `utils/timezone`, `timezoneResolver.ts` deleted, mislabel fixed; +28 unit tests; normalizeTimezone + URLPill output proven byte-identical) |
| 013  | Unify rate-limit + community-budget into one authority, one reset formula; enforce the per-IP gate on **all** LLM routes | P2 | M | 005, 003 (soft) | DONE (new `lib/limits.ts`: `evaluateLimits` composes budget+per-IP with budget checked first, fail-open preserved; per-IP gate spread to summarize/detect-urls/resolve-timezone (charge after success); parse contract byte-identical; `/api/usage` returns unified `reason`/`budget`/`ipRate` + back-compat top-level fields; third reset formula in `CommunityLimitScreen` deleted; +16 tests) |
