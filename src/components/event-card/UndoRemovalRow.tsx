  ...
  return (
    ...
    >
      ...
      <button
        ...
        onClick={onUndo}
        className="group text-sm text-black inline-flex items-center gap-1.5 flex-shrink-0 focus:outline-none focus:ring-2 focus:ring-black"
        aria-label={`Undo removing ${title}`}
      >
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 15L3 9m0 0l6-6M3 9h12a6 6 0 010 12h-3" />
        </svg>
        <span className="underline group-hover:no-underline">Undo</span>
      </button>
    </div>
  );
}
