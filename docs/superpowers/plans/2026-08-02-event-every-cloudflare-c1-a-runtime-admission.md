...
`scripts/run-c1-a-worker-e2e.ts` keeps its public command under Bun: it preflights fixed `.open-next` and `.wrangler`, owns/removes them in `finally`, installs the process boundary, hashes authored inputs, and builds OpenNext under `createCloudflareChildEnvironment`. After that build, it starts exactly one private `node --no-warnings --experimental-strip-types scripts/run-c1-a-worker-e2e.ts --internal-harness <256-bit-token>` child. The parent passes that same invocation-owned 32-byte random hex token in both exact private argv and scrubbed environment, but deletes every Node/Bun/native-loader/proxy injection control, including `NODE_OPTIONS`; the offline preload must not run in this child because it would reject the intentional canary before MSW observes it. The closed public parser rejects the private mode, while the internal mode requires Node >=22, closed args, a 256-bit token, and exact argv/environment token equality. The runner uses an explicit `.ts` local import and an `import.meta.dirname`/`fileURLToPath` fallback so Node 22 can load it directly.

The Node child installs `installCloudflareProcessBoundary(root)` before importing MSW/Wrangler, then exclusively imports `msw` and dynamically imports local Wrangler, installs the MSW and `net`/`tls` zero-egress guard, creates/listens to the harness, runs the exact canary, and owns the `127.0.0.1:8788` bridge. It records only `{ method, url }` and accepts exactly one `POST https://openrouter.ai/api/v1/chat/completions`, reads no provider headers/body, and throws before delegating every non-loopback Node `net`/`tls` attempt. The canary accepts only status 502 with `body.code === "scan_provider_failed"`; its human-safe `error` text is deliberately ignored. After the canary succeeds, the child writes only the fixed `C1_A_WORKER_E2E_READY` readiness token. The synthetic values remain fixed non-secret harness data and do not alter `wrangler.jsonc`; no live provider call is permitted because MSW intercepts the request before a network socket.

The Bun parent starts its absolute 30-second startup deadline before child acquisition and keeps abort-safe acquisition handoff records until every late build/child acquisition is settled and cleaned. Readiness accepts arbitrary fragmentation of exactly one token line, rejects prefixes/extra output, then keeps stdout draining; all stdout/stderr drain rejections are observed, bounded/redacted, and converted to fixed cleanup errors. It starts local Playwright only after readiness, bounds it at 180 seconds, watches the Node child throughout the browser run, treats any premature child exit as terminal, and on every cleanup path sends TERM then KILL only if the TERM grace loses before proving port 8788 is closed. It retains authored-hash verification and removal of only invocation-owned outputs. Browser acceptance therefore still exercises the actual OpenNext bundle in workerd through Wrangler's test harness, never Next dev or `wrangler dev`.

The owned Node bridge never passes a Node-global `Request` across the Wrangler/Miniflare realm boundary. It converts each inbound Node request to `{ input: string, init: RequestInit }`, calls `harness.fetch(input, init)`, copies headers/path/query/method, and preserves `Readable.toWeb(req)` plus `duplex: 'half'` only for non-GET/HEAD methods. The conversion itself and dispatch are inside one sanitized failure boundary: either failure returns exactly status 502 with JSON `{"error":"Worker bridge dispatch failed.","code":"worker_bridge_dispatch_failed"}` and never emits native error text. Immediately after the bridge binds and before `C1_A_WORKER_E2E_READY` is written, the child performs one bounded, abortable loopback `GET /` through that bridge; it does not inspect/log the body, cancels it, and requires status exactly 200. The Node child owns a lifecycle `AbortController`; its `SIGTERM`/`SIGINT` handlers abort it and resolve the child stop wait. The root smoke receives that signal, fails before fetch when already aborted, and readiness checks the signal again after smoke, so termination can never write readiness. The detached unref delay used by startup/browser deadlines is also the default TERM grace race delay, so a graceful child exit cannot leave a referenced two-second timer alive.
...
The Node-child topology under **Strict-offline Worker browser runner** is binding and supersedes the preceding historical current-process state summary: its parent states are `preflight → process-boundary-installed → build → harness-child-started → harness-ready → http-ready → playwright-started → playwright-settled → playwright-stopped → harness-child-stopped → port-closed → outputs-removed`. Runner tests retain the prior collision, signal-at-each-acquisition, late-acquisition, authored-hash, partial-cleanup aggregation, port-failure, drain-failure, and native-failure-sanitization proofs, and add Node-22 version/module-load/private-auth, token framing, guarded-socket, startup, Playwright-deadline, bridge string/init conversion, streaming POST, fixed-502 for both dispatch and conversion failures, pre-aborted and in-flight lifecycle root-smoke termination/cleanup, root-smoke ordering, and unref-grace proofs. They do not invoke the live harness/browser/provider path.
...
There is one independent C1-A Worker E2E prerequisite before Task 10: independently rereview and
commit exactly `docs/superpowers/plans/2026-08-02-event-every-cloudflare-c1-a-runtime-admission.md`,
`scripts/run-c1-a-worker-e2e.ts`, and `scripts/run-c1-a-worker-e2e.test.ts`. It changes no task
manifest or accepted manifest hash, does not stage any Task 10 path, and the Task 10 review/commit
must occur only after this prerequisite is independently accepted as prior Task 1/2 ownership.
