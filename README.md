...
The app is open to everyone. Usage shares a daily community budget
(`DAILY_BUDGET_USD`, default $5, resets midnight UTC) metered
...
(saved to Cloudflare D1 and confirmed via Resend). There is no privileged
bypass; community traffic requires its dedicated fail-closed provider key.
...
| `OPENROUTER_COMMUNITY_KEY` | Required dedicated key for community traffic (recommended: $5 limit, daily reset) |
